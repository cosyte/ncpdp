...
- **Product/Service ID Qualifier (436-E1) carries no label.** The qualifier and the product id are
  ...
  no publicly-available artifact establishing one could be obtained: `product` is `{ id, qualifier }`
  and nothing else. Absence of a meaning never means the value is invalid. Which fields do and do not
