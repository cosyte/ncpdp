...
# Anything a consumer reads (a GitHub release body, README.md, KNOWN-LIMITATIONS.md,
# docs-content/, the npm package description) describes what the software does and what
# changed. It must never carry our internal bookkeeping: item identifiers (`NCPDP-7`,
# `CCDA-P7`), "Phase 8" / "roadmap Phase K", sweep and programme names, ADR numbers,
# internal repo paths, or process commentary about how the artifact came to exist. Source
# of truth: the meta-repo's `documentation/conventions.md`, "No internal project
# bookkeeping on a public surface". The founder's words: "The releases should also not
# speak on anything regarding phases, etc. That has no relevance to the user consuming it.
# This goes for readmes and documentation as well."
#
...
# `(NCPDP-10)` into a README. A documented rule governs whoever reads it; a gate governs
# everyone.
#
...
#     which is the reference implementation the sibling parsers copy. THE SHAPE, NOT THE
#     FILE: that copy carries HL7-specific machinery (the `CSP` Clinical Study Phase field
#     names, the `PKG` Item Packaging segment, HL7 v2 table numbers written `HL7-0396`)
#     which is dead weight here, and it is missing the collision that bites hardest in THIS
#     repo. What is carried across verbatim because it is genuinely cross-repo: the prefix
#     list, the paragraph-join second pass, the doc-comment third pass, the silent-green
#     route closures, and the NEGATIVE self-tests. What is re-derived for NCPDP: the scan
#     surface, the standards-designation exclusions, the phase rule's clinical guards, and
#     every self-test sample.
#
...
#       WHERE THAT TRAP IS SHARPEST, because the token being stripped is the name of the
#       standard the package parses. `NCPDP-7` is one of our items; `NCPDP-SCRIPT`,
#       `NCPDP-TELECOM` and `NCPDP-D.0` are the designations of the standards a consumer
#       came here to read about. Add the sibling designations a pharmacy page reaches for
#       (`HL7-V2`, `FHIR-R4`, `X12-837P`, `DICOM-SR`) and the NCPDP field references that
#       are typographically identical to an identifier but open with a digit (`439-E4`,
#       `111-AM`, `511-FB`, `407-D7`), and a shape rule destroys the reference material it
#       was added to protect. The cost of keying on prefixes is that a NEW PROGRAMME MEANS
...
#       `docs-content/` through. IT IS ALSO WHAT KEEPS THIS REPO'S OWN LICENSING PROSE
#       INTACT: `NCPDP-copyrighted`, `NCPDP-derived` and `NCPDP-licensing` appear eleven
#       times across the public surface and a case-insensitive rule calls every one of
#       them a violation. Leading digits are fine too: `835`, `271` and `837` open X12
#       headlines legitimately, and NCPDP's own transaction codes are `B1`, `B2`, `B3` and
#       `E1`, so nothing here keys on a leading number.
#
...
#   * KNOWN-LIMITATIONS.md the "do not over-trust" list. NOT in package.json `files`, so
#                          it is not in the tarball, but README.md links to it as the
#                          honest boundary list and it is the second thing a consumer
#                          reads. hl7 has no such file, so this entry is ncpdp's own.
...
#                          `docs/adr/0001-xml-parser.md` records why `fast-xml-parser` is
...
#         shared limit. The ONE deliberate divergence in this copy is `SYNTH`, removed with
#         its reason at the list itself.
...
#         heading ending in the word. THIS SHAPE WAS THE MAJORITY OF THIS REPO'S BACKLOG
#         (measured: of the doc-comment hits, more ended a clause than continued one), so
#         it is a large residual here, not a footnote. A rule for the determiner form was
...
#         prefix, and that is trap (1) with a sharp edge in a pharmacy package: legacy
#         SNOMED RT codes are axis-prefixed in exactly that shape (`D-13000` topography,
#         `T-32000`, `M-80003`), and NCPDP's own field references are two characters after
#         a hyphen (`439-E4`, `111-AM`). This repo does not use `D-NN` labels today;
#         the non-catch is stated so a future one is a known gap rather than a surprise.
...
#         means portion and is nobody's jargon. Two live instances were found by this pass
#         on the tree it shipped with, both in `src/telecom/` doc comments describing a
#         substring of the wire message. BOTH WERE REWRITTEN ("portion", "substring"), and
#         the rule was NOT narrowed: a narrowing has no self-test to hold it, and "portion"
...
#
#   * `SYNTH` -- and this one is specific to THIS repo, found by measuring rather than by
#     reading hl7's copy. Every runnable example in this package's README, cookbook and
#     quickstart uses SYNTHETIC message identifiers that say so in their value:
#     `SYNTH-MSG-0001`, `SYNTH-MSG-0002`, `SYNTH-1`. That naming is not decoration, it is
#     the PHI discipline made visible in the sample a consumer copy-pastes. With `SYNTH`
#     in the list this gate reds on nine lines of correct example data and tells the
#     remediator to rename it, which is trap (1) arriving through the LIST rather than
#     through the pattern. Dropping it costs one meta-repo prefix that has never been
#     minted as an ncpdp item and could not appear in an ncpdp doc if it were. Asserted in
#     NEGATIVE[0] and SRC_NEGATIVE[0] so a later "resync with hl7" cannot quietly delete
...
# projects. `NCPDP` IS THE ONE THAT MATTERS HERE: this is the package whose own standard
# is named by the token the identifier rule strips, so `NCPDP-SCRIPT`, `NCPDP-TELECOM` and
# `NCPDP-D.0` have to survive while `NCPDP-7` does not. There is no shape that separates
# them, so the separation is an explicit, reviewable exclusion list, which is the same
# bargain as keying on prefixes in the first place: it must be extended by hand, and that
# is the cheaper mistake. Every entry here is asserted in this rule's NEGATIVE sample.
#
# `NCPDP-F\d` covers the vF6 Telecom designation; `NCPDP-D\.\d` covers vD.0. Neither
# collides with an item, because items in this repo are `NCPDP-<n>` with a bare digit.
#
# HL7's `HL7-\d{3,4}` ARM IS DELIBERATELY DROPPED. In the hl7 copy it exempts HL7 v2 table
# numbers (Table 0396, Table 0003) written with a hyphen, which are reference material an
# HL7 parser's docs cannot do without. An NCPDP parser's docs have no such table
# convention: measured on this tree, `HL7-` appears zero times on the public surface and
# in `src/` doc comments. Carrying the arm would exempt a shape this repo never writes and
# would weaken the rule against a real `HL7-<digits>` item identifier leaking in from a
# sibling repo's release note. That is porting the FILE rather than the SHAPE.
...
#
# The collisions this rule has to survive in a PHARMACY package are not hypothetical.
# NCPDP Telecom field references are written `<number>-<two chars>` (`439-E4` Reason for
# Service, `111-AM` Segment Identification, `511-FB` Reject Code, `407-D7` Product/Service
# ID) and are typographically identical to an item identifier apart from the leading digit,
# which is why nothing here keys on a leading number. NDC product codes, the `B1`/`B2`/`B3`
# and `E1` transaction codes, and the synthetic example message ids all sit in the same
# neighbourhood. All are asserted in NEGATIVE[0] so a later "simplification" cannot quietly
# drop them.
...
#   when followed by trial vocabulary. These are MORE reachable in a pharmacy package than
#   in an HL7 one, not less: drug development is measured in Phase I/II/III trials, an
#   acute-phase reactant is ordinary clinical English, hormone therapy is dosed against the
#   luteal and follicular phases, and a compounded preparation genuinely has a liquid
#   phase. A bare `Phase III` is still flagged, because it is genuinely ambiguous with an
#   internal single-letter item and a loud red on a rare line beats a silent hole.
#
...
#   no NCPDP counterpart: neither SCRIPT nor the Telecommunication Standard has a study
#   phase segment, and measured on this tree those four phrases appear zero times. Carrying
#   them would exempt a construction this repo cannot write while widening the hole in
#   residual (vi). `number` is dropped with them for the same reason.
#
# `phase[ -]` rather than `phase ` is kept: `Phase-L` was live in hl7's docs and slipped a
# space-only rule, and `phase-by-phase` was live in THIS repo's docs and slipped it too.
#
# `phases?` RATHER THAN `phase` IS AN NCPDP ADDITION, measured rather than copied. hl7's
# copy matches the singular only, and this repo's docs-content read "(Phases 6-7)" and
# "the phases that follow", which a singular rule walks straight past. Widening the stem
...
# reader's than ours.
#
# THE IMAGING-NOUN EXCLUSION IS KEPT VERBATIM even though this package documents pharmacy
# claims and ePrescribing and reaches for none of it. It only ever EXCLUDES, so it cannot
# cause a miss of our jargon; diverging from the sibling copies to delete an unused
# alternation would make the copies harder to diff for no safety gained (residual (i)). It
# is grounded in @cosyte/dicom's generated tag dictionary (SliceThickness, SliceLocation,
...
# @cosyte/ncpdp has no meta-repo and no such file. Keyed on the known meta-repo paths, not
# on a `dir/file.md` shape, for exactly the reason trap (1) gives -- this package's own
# pages legitimately cite `docs-content/spec-notes-structured-sig.md`, which a shape rule
# would take with it.
...
#
# MEASURED, not assumed, on the tree that added this pass: SIX runtime warning messages
# carried "this phase" into a consumer's log --
#   "Transaction Response Status is not modeled this phase; preserved verbatim ..."
#   "Reject Code is not recognized this phase; ..."
#   "Segment code ${segmentId} is not modeled this phase; preserved verbatim."
#   "Transmission carries N group-separated transactions; only the first is decoded this phase."
#   "Request DUR/PPS Reason For Service code is not recognized this phase; ..."
#   "SCRIPT transaction <X> is not modeled in this phase; surfaced as unsupported."
# All six are gone. NONE OF THEM WAS FOUND BY A RULE, AND THIS PASS WOULD NOT HAVE FOUND
# THEM EITHER. Say that plainly rather than let the list above imply otherwise: every one
# of the six ends the clause at `phase` (`phase;`, `phase.`), which is residual (vi), the
# shape rule 2 deliberately does not cover because the determiner-plus-`phase` form
# collides with ordinary clinical English. They were reviewer catches, and this pass's own
# POSITIVE sample for rule 2 had to be rewritten to something the rule actually matches --
# which is the self-test doing its job on the author of the pass.
#
# SO WHAT IS THIS PASS WORTH? It closes the SURFACE, not that one shape. An item identifier
# (`(NCPDP-9)`), an ADR number, a meta-repo path, a `Phase 9` with a following token, or a
...
# strings is the obvious place for one. All six rules over all 2,528 double-quoted and
# backtick literals in tracked `src/`, on the remediated tree: ZERO matches. Import
# specifiers, the `NCPDP_TELECOM_*` warning-code constants (underscored, so rule 1's
# hyphen requirement never fires), the field ids and the paraphrase tables all pass
...
NEGATIVE[0]='NCPDP-SCRIPT and NCPDP-TELECOM, the vD.0 designation NCPDP-D.0 and the vF6 one NCPDP-F6, NCPDP-copyrighted text, NCPDP-derived labels and NCPDP-licensing posture, Reason for Service 439-E4, Segment Identification 111-AM, Reject Code 511-FB, Product/Service ID 407-D7, transaction codes B1 B2 B3 and E1, the example ids SYNTH-MSG-0001 and SYNTH-MSG-0002 and SYNTH-1, ICD-10-CM P00-P96, FHIR-bridge stability, docs-content/ layout, HL7-defined tables, HL7-V2 and HL7-CDA, FHIR-R4, DICOM-SR, X12-837P and X12-005010, 835 remittance'
NEGATIVE[1]='A Phase III oncology trial and a Phase II study; the clinical phases of a drug programme; the acute phase reactant; luteal phase dosing and follicular phase dosing; the liquid phase of a compounded preparation; the adapter stays in phase with the source system and is out of phase'
...
# source: NCPDP field references in doc comments, the standards designations, and
# TypeScript that reads like our jargon (`raw.slice(0, 56)`, `value.padEnd(n).slice(0, n)`).
...
SRC_NEGATIVE[0]='NCPDP-SCRIPT and NCPDP-TELECOM and NCPDP-D.0 and NCPDP-F6, NCPDP-copyrighted prose, Reason for Service 439-E4, Segment Identification 111-AM, Reject Code 511-FB, Product/Service ID 407-D7, Prescription Service Reference Number 402-D2, Date of Service 401-D1, Cardholder ID 302-C2, transaction codes B1 B2 B3 E1, the example ids SYNTH-MSG-0001 and SYNTH-1, ICD-10-CM P00-P96, FHIR-bridge stability, HL7-defined tables, HL7-V2, FHIR-R4, DICOM-SR, X12-837P'
SRC_NEGATIVE[1]='A Phase III oncology trial and a Phase II study; the clinical phases of a drug programme; the acute phase reactant; luteal phase dosing; the liquid phase of a compounded preparation; the adapter stays in phase with the source system and is out of phase'
SRC_NEGATIVE[2]='ADR is not a segment identifier, and 0015 alone is a value'
SRC_NEGATIVE[3]='The slice thickness and the number of slices are DICOM attributes, each slice location is too; raw.slice(0, 56) and value.padEnd(n).slice(0, n) are TypeScript; and the phase of the clinical study, the phase of illness and each phase of the trial are the reader words this rule must not touch'
...
STR_NEGATIVE[0]='NCPDP_TELECOM_UNKNOWN_SEGMENT and NCPDP_SCRIPT_SIG_STRUCTURED_LOSSY and NCPDP_TELECOM_MULTI_TRANSACTION_TRUNCATED, ./tokenize.js and ../common/decimal.js, NCPDP-SCRIPT and NCPDP-TELECOM and NCPDP-D.0, Reason for Service 439-E4, Segment Identification 111-AM, Reject Code 511-FB, Cardholder ID 302-C2, B1 B2 B3 E1, SYNTH-MSG-0001'
STR_NEGATIVE[1]='Transaction Response Status is not modeled by this parser; preserved verbatim, disposition reads unknown (never paid). A Phase III trial and the acute phase reactant are out of scope, and the reader stays in phase with the source system.'
STR_NEGATIVE[2]='ADR is not a segment identifier, and 0001 alone is a value'
STR_NEGATIVE[3]='Transmission carries 3 group-separated transactions; only the first is decoded. The slice thickness and the number of slices are DICOM attributes.'
...
SURFACE_PATHS=(README.md KNOWN-LIMITATIONS.md TRADEMARKS.md LICENSE docs-content)
...
#     entry points (`.`, `/script`, `/telecom`, `/profiles`, `/common`), so "does it reach
#     a declaration file" is five questions, not one.
