...
the test files vitest would actually run and fails on any shortfall. It asks vitest for its resolved
...
whatever it is named, with no exemption of any kind: renaming a suite out of the conventional test
suffix does not remove it, and a helper may not live there. The repo's one such helper moved to
...
`vitest run`. Selection is also necessary rather than sufficient: a selected test that asserts
