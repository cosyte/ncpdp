...
## Relocated from `CLAUDE.md`, 2026-09-06

`CLAUDE.md` had grown to 471 lines against the 300-line ceiling the estate's drift manifest
declares for an agent-context doc, and the drift check reports that overrun as a finding against
this repository. The blocks below came out of it BYTE-VERBATIM under the same rule the 2026-08-04
split used and this file's preamble restates: put the paragraph THERE and a one-line imperative
HERE. Nothing was deleted, nothing was shortened and nothing was reworded. The cursor keeps each
block's imperative heads, verbatim, and points here for the incident, the measurements, the shas
and the negative controls. Only the SHAPE of the cursor changed, which is the only thing the
ceiling is about.

### The diagnostic traps

Relocated out of the `Diagnostics are built from a frozen registry` bullet under `## Status`.
The cursor keeps the imperatives; every incident, list provenance and slot count is here.
...
### The PHI scanner traps

Relocated out of the `PHI commit-gate armed on both wire formats` bullet under `## Status`. Twelve
traps, each the record of a measured defect in `scripts/phi-scan.ts`.
...
### The wire-code label traps

Relocated out of the `A wire-code label ships with the artifact` bullet under `## Status`, with both
of its sub-traps.
...
### The conformance statement traps

Relocated out of the `The decoded version set is written down ONCE` bullet under `## Status`. Each
of the four names the published page it shipped wrong on.
...
### The release caller traps

Relocated out of the `The release caller's four token grants are gated` bullet under `## Status`.
...
### The required-checks traps

Relocated out of `## Required checks on `main``. Nine rules, each carrying what it cost and the
pointer to the section that grounds it.
...
### The attw wrapper traps

Relocated out of the `attw` bullet under `## Engineering Guardrails`. The two nets, what each
catches, and the refusals that keep the post-check readable.
...
Relocated out of discipline 4 under `## Standing disciplines (every change)`.
