...
/**
 * The result of parsing a NCPDP payload.
 *
...
export interface ParsedNcpdp {
  ...
  readonly warnings: readonly NcpdpWarning[];
}
...
/**
 * Options for {@link parseNcpdp}.
 *
 * The parser is **lenient by default** (Postel's Law): vendor quirks become {@link NcpdpWarning}s
 * rather than failures. Set `strict` to escalate every Tier-2 deviation to a thrown error.
 */
export interface ParseNcpdpOptions {
  /** When `true`, escalate every Tier-2 deviation to a thrown error instead of a warning. */
  readonly strict?: boolean;
}

/**
 * Parse a NCPDP payload into a {@link ParsedNcpdp}.
 *
 * **Lenient by default** — real-world, vendor-quirky input parses rather than throws, emitting
 * {@link NcpdpWarning}s instead (Postel's Law). The complementary serializer (added in a later
 * phase) always emits spec-clean output. Only unrecoverable structural corruption throws (a Tier-3
 * {@link FatalCode}).
 *
 * @param raw - The raw NCPDP payload (`string` for now; `Buffer`/`Uint8Array` support is added
 *   alongside the real parser).
 * @param options - Parse options; see {@link ParseNcpdpOptions}. Lenient unless `strict` is set.
 ...
 *
 * const { value, warnings } = parseNcpdp(raw);
 * for (const w of warnings) console.warn(w.code, w.position);
 * ```
 */
export function parseNcpdp(raw: string, options: ParseNcpdpOptions = {}): ParsedNcpdp {
 ...
 * @example
 * ```ts
 * import { WARNING_CODES } from "@cosyte/ncpdp";
 *
 ...
 * @example
 * ```ts
 * import { FATAL_CODES } from "@cosyte/ncpdp";
 *
 * try {
 *   parseNcpdp(raw);
 * } catch (err) {
